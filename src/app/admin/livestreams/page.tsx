  ...
  const fetchYoutubeInfo = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(data.error || "Yayin bilgileri alinamadi");
      }
    } catch (err) {
        ...
        <div className="bg-[var(--surface)] rounded-xl border border-[var(--border)] overflow-hidden">
          <div className="overflow-x-auto">
            <table className="table w-full">
              <thead><tr><th>Thumbnail</th><th>Baslik</th><th>Izlenme</th><th>Durum</th><th>Islemler</th></tr></thead>
              <tbody>
                {streams.map((stream) => (
                  <tr key={stream.id}>
                    <td>
                      {stream.thumbnailUrl ? (
                        <img src={stream.thumbnailUrl} alt={stream.title} className="w-20 h-12 object-cover rounded-lg" />
                      ) : (
        ...
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-content p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              {/* YouTube URL Input */}
              <div>
                <label className="block text-sm font-medium text-[var(--text-muted)] mb-2">YouTube Yayin URL</label>
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={youtubeUrl}
              ...
              {/* Preview - Bilgiler çekildiğinde veya edit modunda göster */}
              {formData.embedUrl && (
                <div className="bg-[var(--background)] rounded-lg p-4 border border-[var(--border)]">
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-green-400 mt-2">Yayin bilgileri alindi</p>
                    </div>
                  </div>
                </div>
              )}
              ...
              <div className="flex items-center gap-2 p-3 bg-[var(--background)] rounded-lg border border-[var(--border)]">
                ...
                />
                <label htmlFor="isLive" className="text-sm text-white font-medium flex items-center gap-2">
                  <span className={`w-2 h-2 rounded-full ${formData.isLive ? 'bg-red-500 animate-pulse' : 'bg-gray-500'}`}></span>
                  Canli Yayin Aktif
                </label>
              </div>
            </div>
            ...
            <div className="flex gap-3 mt-6">
              <button type="button" onClick={handleSubmit} disabled={!formData.embedUrl && !editingItem} className="btn btn-primary flex-1 disabled:opacity-50">Kaydet</button>
