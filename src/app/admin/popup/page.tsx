...
export default function PopupPage() {
  const [popups, setPopups] = useState<PopupSponsor[]>([]);
  const [showModal, setShowModal] = useState(false);
  const [editingItem, setEditingItem] = useState<PopupSponsor | null>(null);
  ...
  const fetchPopups = async () => {
    try {
      const res = await fetch("/api/popup-sponsors");
  ...
  const openEditModal = (item: PopupSponsor) => {
    setEditingItem(item);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/popup-sponsors/${id}`, { method: "DELETE" });
    ...
    <AdminLayout>
      <div className="p-4 md:p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl font-bold text-white">Popup Sponsorlar</h1>
            <p className="text-[var(--text-muted)] text-sm">{popups.length} popup</p>
          </div>
          <button type="button" onClick={openAddModal} className="btn btn-primary">+ Yeni Popup</button>
        </div>
                  ...
                  <tr key={popup.id}>
                    ...
                    <td>
                      <div className="flex gap-2">
                        <button type="button" onClick={() => openEditModal(popup)} className="btn btn-secondary text-xs py-1 px-3">Duzenle</button>
                        <button type="button" onClick={() => handleDelete(popup.id)} className="btn btn-danger text-xs py-1 px-3">Sil</button>
                      </div>
                    </td>
                  </tr>
                ))}
                {popups.length === 0 && <tr><td colSpan={5} className="text-center py-8 text-[var(--text-muted)]">Henuz popup yok</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-content p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-lg font-bold text-white">{editingItem ? "Popup Duzenle" : "Yeni Popup"}</h3>
